      ...
      if (!vehicles.length) {
        ...
        if (seedVehicles.length) {
          const { data: seeded, error: seedError } = await supabase
            .from(FLEET_VEHICLE_TABLE)
            .upsert(seedVehicles.map((vehicle) => ({
              plate: vehicle.plate,
              site: vehicle.site,
              active: true,
...
if (!adminHtml.includes("fleet-vehicle-v75-script")) {
  const fleetScript = `
  <script id="fleet-vehicle-v75-script">
   (() => {
    let fleetVehicles = [];
    const normalizePlate = (value) => String(value || "").trim().toUpperCase().replace(/\s+/g, " ").replace(/[^A-Z0-9 ]/g, "");
    const get = (id) => document.querySelector(id);
    async function loadFleetVehicles() {
      const list = get("#fleetVehicleList");
      if (!list) return;
      try {
        const response = await fetch("/api/vehicles", { cache: "no-store" });
        const result = await response.json();
        ...
        list.innerHTML = '<article class="empty-state">' + (error.message || "No se pudo cargar la flota.") + '</article>';
      }
    }
    function renderFleetVehicles() {
      const list = get("#fleetVehicleList");
      if (!list) return;
      const q = normalizePlate(get("#fleetVehicleSearch")?.value || "");
      const vehicles = fleetVehicles.map((vehicle) => ({ plate: normalizePlate(vehicle.plate || vehicle), site: vehicle.site || "all", active: vehicle.active !== false })).filter((vehicle) => vehicle.plate && vehicle.active && (!q || normalizePlate(vehicle.plate).includes(q))).sort((a, b) => a.plate.localeCompare(b.plate));
      const count = get("#fleetVehicleCount");
      if (count) count.textContent = String(vehicles.length);
      list.innerHTML = vehicles.length ? vehicles.map((vehicle) => '<article class="fleet-vehicle-row"><div><strong>' + vehicle.plate + '</strong><span>' + (vehicle.site === "all" ? "DRP3 + DSU1" : vehicle.site) + '</span></div><button type="button" data-remove-fleet-vehicle="' + vehicle.plate + '">Quitar</button></article>').join("") : '<article class="empty-state">No hay vehiculos.</article>';
    }
    async function addFleetVehicle() {
      const input = get("#fleetVehiclePlate");
      const plate = normalizePlate(input?.value || "");
      const site = get("#fleetVehicleSite")?.value || "all";
      if (!plate) return alert("Escribe una matricula valida.");
      const response = await fetch("/api/admin/vehicles", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ plate, site }) });
      const result = await response.json();
      if (!response.ok || result.ok === false) return alert(result.error || "No se pudo guardar el vehiculo.");
      fleetVehicles = result.vehicles || [];
      input.value = "";
      renderFleetVehicles();
    }
    document.addEventListener("DOMContentLoaded", () => {
      get("#addFleetVehicle")?.addEventListener("click", addFleetVehicle);
      get("#fleetVehiclePlate")?.addEventListener("keydown", (event) => { if (event.key === "Enter") addFleetVehicle(); });
      get("#fleetVehicleSearch")?.addEventListener("input", renderFleetVehicles);
      get("#fleetVehicleList")?.addEventListener("click", async (event) => {
        const button = event.target.closest("[data-remove-fleet-vehicle]");
        if (!button) return;
        const response = await fetch("/api/admin/vehicles/" + encodeURIComponent(button.dataset.removeFleetVehicle), { method: "DELETE" });
        const result = await response.json();
        if (!response.ok || result.ok === false) return alert(result.error || "No se pudo quitar el vehiculo.");
        fleetVehicles = result.vehicles || [];
        renderFleetVehicles();
      });
      loadFleetVehicles();
      setInterval(loadFleetVehicles, 60000);
    });
   })();
  </script>
`;
